...
function definedFilesOf(name) {
  const files = [];
  if (!workspaceIndex || typeof name !== "string" || !name) return files;
  for (const [f, data] of workspaceIndex.files) {
    if (data.defs.some((d) => d.name === name)) files.push(f);
  }
  return files.sort();
}
...
async function walkSourceFiles(root, limit, out, depth, deadline, skipped) {
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) {
      ...
      await walkSourceFiles(p, limit, out, depth + 1, deadline, skipped);
    } else if (e.isFile()) {
...
const OPS = {
  ...
  async find_references({ name, file, limit, offset }) {
    ...
    if (fileFilter) {
      ...
      const keys = [...workspaceIndex.files.keys()];
      keyFilter =
    ...
    const all = [];
    for (const [f, data] of workspaceIndex.files) {
      if (keyFilter && f !== keyFilter) continue;
      const defLines = new Set(data.defs.filter((d) => d.name === name).map((d) => d.start_line));
      for (const r of data.refs) {
        if (r.name !== name) continue;
        all.push({ file: f, line: r.line, kind: defLines.has(r.line) ? "definition" : "reference" });
      }
    }
    ...
    return {
      name,
      __debug: { fileArg: file ?? null, fileFilter, keyFilter, keyCount: workspaceIndex.files.size },
      total: all.length,
      returned: results.length,
      offset: page.offset,
  ...
  async callers({ name, file, language, limit, offset }) {
    ...
    for (const [f, data] of workspaceIndex.files) {
      ...
      if (language && data.lang !== language) continue;
      for (const c of data.calls ?? []) {
        if (c.callee !== name) continue;
        all.push({ file: f, lang: data.lang, line: c.line, caller: c.caller ?? "(top-level)", ...(c.recv ? { recv: c.recv } : {}) });
      }
    }
    all.sort((a, b) => (a.file === b.file ? a.line - b.line : a.file < b.file ? -1 : 1));
    const results = all.slice(page.offset, page.offset + page.limit);
  ...
  async callees({ name, file, language, limit, offset }) {
    ...
    for (const [f, data] of workspaceIndex.files) {
      ...
      for (const c of data.calls ?? []) {
        ...
        all.push({ file: f, lang: data.lang, line: c.line, callee: c.callee, ...(c.recv ? { recv: c.recv } : {}) });
      }
    }
    all.sort((a, b) => (a.file === b.file ? a.line - b.line : a.file < b.file ? -1 : 1));
    const results = all.slice(page.offset, page.offset + page.limit);
