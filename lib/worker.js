...
import { COMPLEXITY_SPEC, CALL_SPEC, countDecisions, callsOf, halveInPlace, recvText } from "./analysis.js";
...
async function compileUserQueries(langObj, dir) {
  ...
  for (const f of names) {
    try {
      ...
      out.push(compileQuery(langObj, source));
    } catch {}
  }
  return out;
}

const DIRECTIVE_LINE_RE = /^[ \t]*\(\s*#[\w-]+!(?:\s+@[\w.-]+)?(?:\s+"(?:[^"\\]|\\.)*")?\s*\)[ \t]*\r?\n?/gm;
const STRIP_DIRECTIVE_RE = /^[ \t]*\(\s*#strip!\s+@([\w.-]+)\s+"((?:[^"\\]|\\.)*)"\s*\)[ \t]*\r?\n?/gm;

function compileQuery(langObj, source) {
  const strips = new Map();
  let m;
  STRIP_DIRECTIVE_RE.lastIndex = 0;
  while ((m = STRIP_DIRECTIVE_RE.exec(source)) !== null) {
    try {
      strips.set(m[1], new RegExp(m[2].replace(/\\(.)/g, "$1"), "g"));
    } catch {}
  }
  const cleaned = source.replace(DIRECTIVE_LINE_RE, "");
  const q = new Query(langObj, cleaned);
  return { q, strips };
}
...
async function getLang(name) {
  ...
  if (spec.definitions) {
    try {
      builtin = compileQuery(langObj, await fs.readFile(spec.definitions, "utf8"));
    } catch {
      builtin = null;
    }
  }
  let refQueries = [];
  let callRefs = false;
  if (spec.upstreamTags) {
    try {
      const src = await fs.readFile(spec.upstreamTags, "utf8");
      refQueries = [compileQuery(langObj, src)];
      callRefs = src.includes("@reference.call");
    } catch {
      refQueries = [];
      callRefs = false;
    }
...
function defsOf(tree, defQueries, deadline) {
  if (defQueries && defQueries.length) {
    const out = [];
    for (const { q, strips } of defQueries) {
      for (const m of q.matches(tree.rootNode)) {
        ...
        for (const c of m.captures) {
          if (c.name === "name") {
            if (!nameCap) nameCap = c;
          } else if (c.name === "doc") {
            if (!docCap) docCap = c;
          } else if (!defCap && (c.name.startsWith("definition.") || /\.def$/.test(c.name))) {
            defCap = c;
          }
        }
        if (!defCap) {
          for (const c of m.captures) {
            if (c.name === "name" || c.name === "doc") continue;
            if (defName(c.node)) {
              defCap = c;
              break;
            }
        ...
        if (!defCap) continue;
        const nameNode = nameCap && nameCap.node.id !== defCap.node.id ? nameCap.node : defName(defCap.node);
        if (!nameNode) continue;
        const kind = defCap.name.startsWith("definition.") ? defCap.name.slice(11) : defCap.name.replace(/\.def$/, "");
        let doc = null;
        if (docCap) {
          doc = docCap.node.text;
          const strip = strips.get(docCap.name);
          if (strip) doc = doc.replace(strip, "");
          doc = doc.trim();
        }
        out.push({ name: nameNode.text, kind, node: defCap.node, ...(doc ? { doc } : {}) });
      }
    }
    return out;
  }
...
function refTagsOf(tree, refQueries) {
  const out = [];
  for (const { q } of refQueries) {
    for (const m of q.matches(tree.rootNode)) {
      ...
      let nameCap = null;
      for (const c of m.captures) {
        if (!refCap && c.name.startsWith("reference.")) refCap = c;
        else if (!nameCap && c.name === "name") nameCap = c;
      }
      if (!refCap || !nameCap || !nameCap.node) continue;
      out.push({ kind: refCap.name.slice(10), name: nameCap.node.text, node: refCap.node, nameNode: nameCap.node });
    }
  }
  return out;
}

function extractCalls(tree, lang, entry, defs) {
  if (entry.callRefs && entry.refQueries && entry.refQueries.length) {
    const spec = CALL_SPEC[lang];
    const tags = refTagsOf(tree, entry.refQueries);
    const out = [];
    for (const t of tags) {
      if (t.kind !== "call" && t.kind !== "class") continue;
      let callNode = t.node;
      let depth = 0;
      while (callNode && !spec.calls.includes(callNode.type) && depth < 3) {
        callNode = callNode.parent;
        depth++;
      }
      if (!callNode || !spec.calls.includes(callNode.type)) continue;
      let caller = null;
      for (const d of defs) {
        if (d.node.startIndex <= t.nameNode.startIndex && t.nameNode.endIndex <= d.node.endIndex) {
          if (!caller || d.node.startIndex >= caller.node.startIndex) caller = d;
        }
      }
      out.push({ caller: caller?.name ?? null, callee: t.name, line: t.nameNode.startPosition.row + 1, recv: recvText(callNode) });
    }
    if (out.length) {
      while (out.length > INDEX_MAX_CALLS_PER_FILE) {
        halveInPlace(out);
      }
      return out;
    }
  }
  return callsOf(tree, lang, INDEX_MAX_CALLS_PER_FILE);
}
...
const INDEX_MAX_REFS_PER_FILE = 20000;
const INDEX_MAX_CALLS_PER_FILE = 20000;
...
function sanitizeCachedCalls(v) {
  if (!Array.isArray(v) || v.length > INDEX_MAX_CALLS_PER_FILE) return null;
  const out = [];
  for (const c of v) {
    if (!c) return null;
...
async function indexSingleFile(f, knownKey, targetStore) {
  ...
  try {
    ...
    const rawDefs = defsOf(tree, entry.defQueries);
    ...
    const rs = refsOf(tree).refs;
    const calls = extractCalls(tree, lang, entry, rawDefs);
...
async function buildFileEntry(f, lang, key, deadline, rootAbs) {
  ...
  const rawDefs = defsOf(tree, entry.defQueries, deadline);
  const defs = rawDefs.map((d) => {
    const bases = basesOf(d.node, lang);
    const ret = retOf(d.node, lang);
    return {
  ...
  const { refs: rs, truncated } = refsOf(tree, deadline);
  const { pkg, imports } = importsAndPackageOf(tree, lang);
  const vartypes = vartypesOf(tree, lang);
  const hash = createHash("sha256").update(source).digest("hex").slice(0, 32);
  return {
    entry: { lang, key, defs, refs: rs, calls: extractCalls(tree, lang, entry, rawDefs), hash, pkg, imports, vartypes },
    truncated,
  };
}
...
function refsOf(tree, deadline) {
  ...
  for (const n of walk(tree.rootNode, deadline)) {
    ...
    if (out.length >= INDEX_MAX_REFS_PER_FILE) {
      stride *= 2;
      halveInPlace(out);
      sinceSampled = 0;
      truncated = true;
    }
    if (sinceSampled % stride === 0) out.push({ name: text, line: n.startPosition.row + 1 });
    sinceSampled++;
  }
...
const OPS = {
  ...
  async delete_index({ root }) {
    ...
    if (workspaceIndex && workspaceIndex.root === rootAbs) {
      try {
        workspaceIndex.watcher?.close();
        watcherClosed = true;
      } catch {}
      try {
        workspaceIndex.store?.close();
        storeClosed = true;
      } catch {}
      workspaceIndex = null;
      resolverState = null;
    }
    const inMemoryCleared = watcherClosed && storeClosed;
    const h = createHash("sha256").update(rootAbs).digest("hex").slice(0, 24);
    const base = path.join(CACHE_DIR, h);
    const removed = [];
    for (const suffix of [".db", ".db-wal", ".db-shm", ".json", ".json.migrated"]) {
      const f = base + suffix;
      try {
        await fs.stat(f);
      } catch {
    ...
    return {
      root: rootAbs,
      in_memory_cleared: inMemoryCleared,
      watcher_closed: watcherClosed,
      store_closed: storeClosed,
      cache_files_removed: removed.length,
    };
  },
  ...
  async get_node_types({ file, language }) {
    ...
    let payload = nodeTypesCache.get(lang);
    if (payload === undefined) {
      payload = null;
      const nodeTypesFile = LANGUAGES[lang].nodeTypes;
      if (nodeTypesFile) {
        try {
          const data = JSON.parse(await fs.readFile(nodeTypesFile, "utf8"));
          const named_types = [];
          const anonymous_tokens = [];
          const fields = new Set();
          for (const e of data) {
            if (e.named) {
              named_types.push(e.type);
            } else if (!anonymous_tokens.includes(e.type)) {
              anonymous_tokens.push(e.type);
            }
            if (e.fields) {
              for (const f of Object.keys(e.fields)) fields.add(f);
            }
          }
          payload = {
            lang,
            node_type_count: data.length,
            named_types,
            anonymous_tokens,
            fields: [...fields],
          };
      ...
      nodeTypesCache.set(lang, payload);
    }
  ...
  async callers({ name, file, language, limit, offset }) {
    ...
    const all = [...merged.values()].map((e) => (e.n > 1 ? { ...e.hit, count: e.n } : e.hit));
    all.sort((a, b) => (a.file === b.file ? a.line - b.line : a.file < b.file ? -1 : 1));
    const results = all.slice(page.offset, page.offset + page.limit);
    const defined_in = definedFilesOf(name);
    const resolution = { exact: 0, likely: 0, name: 0 };
    for (const r of all) if (r.confidence) resolution[r.confidence] += r.count ?? 1;
    return {
      name,
      total: totalSites,
      returned: results.length,
      ...
      defined_in,
      resolution,
      ...(defined_in.length === 0 && all.length > 0
        ? { external_only: true, note: `"${name}" is not defined in this workspace — all ${all.length} hits are calls to external/library methods with the same name` }
        : {}),
      ...(defined_in.length > 1
  ...
  async callees({ name, file, language, limit, offset }) {
    ...
    for (const c of rows) {
      totalSites++;
      const key = `${c.file}|${c.line}|${c.callee}`;
      const entry = merged.get(key);
      if (entry) {
        entry.n++;
        continue;
    ...
    return {
      ...
      resolution,
      ...(defined_in.length === 0 && all.length > 0
        ? { note: `"${name}" is not defined in this workspace; callee names may be external/library methods` }
        : {}),
      ...(defined_in.length > 1
