...
function sanitizeCachedFile(e) {
  ...
  return { lang: e.lang, key: e.key, defs, refs, calls };
}
...
async function savePersistedIndex() {
  ...
  try {
    ...
    for (const [f, e] of workspaceIndex.files) {
      files[f] = { lang: e.lang, key: e.key, defs: e.defs, refs: e.refs, calls: e.calls ?? [] };
    }
    const payload = {
...
async function indexSingleFile(f, knownKey) {
  ...
  try {
    ...
    const { tree } = await getTree(f, lang, null, workspaceIndex?.root ?? null);
    ...
    workspaceIndex.files.set(f, { lang, key, defs, refs: rs, calls });
    return true;
  } catch {
    return false;
  }
...
const OPS = {
  ...
  async read_definition({ file, name, language, softDeadlineMs, confineRoot }) {
    const lang = resolveLang(file, language);
    const deadline = softDeadlineOf({ softDeadlineMs });
    ...
    return {
      ...
      defs: hits.map((d) => {
        ...
        return {
          ...
          start_line: d.node.startPosition.row + 1,
          end_line: d.node.endPosition.row + 1,
          truncated: code.length > MAX_CODE,
          code: clip(code, MAX_CODE),
        };
      }),
    };
  },

  async ast_search({ file, pattern, language, softDeadlineMs, confineRoot }) {
    const lang = resolveLang(file, language);
    const deadline = softDeadlineOf({ softDeadlineMs });
    ...
    return {
      ...
      truncated,
      results,
      ...(truncated ? { hint: "pattern too broad; add a child constraint to narrow it" } : {}),
    };
  },
  ...
  async index_workspace({ root, maxFiles, softDeadlineMs }) {
    ...
    const files = [];
    await walkSourceFiles(rootAbs, limit, files, 0, deadline);
    ...
    for (const f of files) {
      ...
      if (cached && cached.lang === lang && cached.key === key) {
        map.set(f, { lang, key, defs: cached.defs, refs: cached.refs, calls: cached.calls });
        reused++;
        continue;
      }
      try {
        const entry = await getLang(lang);
        const { tree } = await getTree(f, lang, deadline, rootAbs);
        ...
        map.set(f, { lang, key, defs, refs: rs, calls: callsOf(tree, lang) });
        parsed++;
      } catch {}
    }
    ...
    for (const [file, data] of workspaceIndex.files) {
      ...
      for (const r of data.refs) {
        ...
        if (results.length < MAX_MATCHES) {
          results.push({ file, line: r.line, kind: defLines.has(r.line) ? "definition" : "reference" });
        }
      }
    }
  ...
  async go_to_definition({ name, file }) {
    if (!workspaceIndex) {
      throw new ToolError("not_found", "no index; run index_workspace first");
    }
    if (typeof name !== "string" || !name) {
    ...
    return {
      name,
      count: all.length,
      truncated: all.length > INDEX_MAX_DEF_RESULTS,
      index_version: workspaceIndex.version,
      defs: all.slice(0, INDEX_MAX_DEF_RESULTS),
    };
  },
  ...
  async callers({ name }) {
    ...
    for (const [file, data] of workspaceIndex.files) {
      for (const c of data.calls ?? []) {
        ...
        total++;
        if (results.length < MAX_MATCHES) {
          results.push({ file, line: c.line, caller: c.caller ?? "(top-level)" });
        }
      }
    }
    results.sort((a, b) => (a.file === b.file ? a.line - b.line : a.file < b.file ? -1 : 1));
  ...
  async callees({ name }) {
    ...
    for (const [file, data] of workspaceIndex.files) {
      for (const c of data.calls ?? []) {
        ...
        total++;
        if (results.length < MAX_MATCHES) {
          results.push({ file, line: c.line, callee: c.callee });
        }
      }
    }
    results.sort((a, b) => (a.file === b.file ? a.line - b.line : a.file < b.file ? -1 : 1));
