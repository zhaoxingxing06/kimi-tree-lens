...
import { COMPLEXITY_SPEC, CALL_SPEC, countDecisions, callsOf, halveInPlace } from "./analysis.js";
...
async function buildFileEntry(f, lang, key, deadline, rootAbs) {
  ...
  const hash = createHash("sha256").update(source).digest("hex").slice(0, 32);
  return {
    entry: { lang, key, defs, refs: rs, calls: callsOf(tree, lang, INDEX_MAX_CALLS_PER_FILE), hash, pkg, imports, vartypes },
    truncated,
  };
}
